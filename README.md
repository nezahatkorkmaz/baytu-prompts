...
A production-grade Prompt-as-Code repository for managing versioned prompts for a real-estate listing system. The registry ships with active Version 3.1 prompts that are localization-safe, include fixed MCP parameters, and enforce validation-only rules.

## What this repo provides

- Versioned prompts for three roles: `default`, `translation`, `assistant`
- Localization integrity and no-fabrication constraints
- MCP fixed parameters (Google Places) defined in one source of truth and embedded in v3.1 prompts
- CI checks: prompt linting, registry validation, required sections, and golden tests
- A prompt registry JSON that maps “latest” aliases to v3.1
...
Active prompts: Version 3.1 (Localization-Safe + MCP fixed params + validation-only).

## How to use

- Pick a role (`default`, `translation`, `assistant`).
- Use the registry at `registry/prompts.index.json` to resolve the `latest` version for each role (currently v3.1).
...
- Required headings and sections for v3.1
...
- Stable selection: use the explicit version path (e.g., `prompts/real_estate/default/v3.1.md`)
